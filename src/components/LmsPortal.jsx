...
export default function LmsPortal({ schoolData, onStartQuiz, onTriggerNotification }) {
  ...
  const currentCourse = lms.find(c => c.id === selectedCourseId) || lms[0] || null;
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      ...
      >
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            ...
            <span className="badge badge-emerald">Interactive Courses</span>
          </div>
          ...
          {lms.map(course => (
            <div
              ...
              style={{
                flex: '1',
                minWidth: '220px',
                background: course.id === (currentCourse?.id || '') ? 'var(--accent-gradient)' : 'var(--bg-card)',
                color: course.id === (currentCourse?.id || '') ? '#ffffff' : 'var(--text-main)',
                border: 'var(--glass-border)',
                borderRadius: 'var(--radius-md)',
                padding: '1rem',
                cursor: 'pointer',
                transition: 'all 0.2s ease',
                boxShadow: course.id === (currentCourse?.id || '') ? 'var(--shadow-lg)' : 'none'
              }}
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
                <span style={{ fontSize: '0.75rem', fontWeight: 700, opacity: 0.9 }}>{course.code || 'CRS'}</span>
                <span style={{ fontSize: '0.75rem', fontWeight: 800 }}>{course.progress || 0}%</span>
              </div>
              <h4 style={{ fontWeight: 800, fontSize: '1.1rem' }}>{course.subject}</h4>
              <span style={{ fontSize: '0.78rem', opacity: 0.8, display: 'block', marginTop: '0.2rem' }}>{course.teacher || 'Faculty Staff'}</span>
            </div>
          ))}
        </div>
      )}
      ...
      {!currentCourse ? (
        <div className="glass-card" style={{ textAlign: 'center', padding: '3.5rem 1.5rem' }}>
          <BookOpen size={48} className="text-blue-400" style={{ margin: '0 auto 1rem auto' }} />
          <h3 style={{ fontSize: '1.25rem', fontWeight: 800 }}>No LMS Courses Available Yet</h3>
          <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem', marginTop: '0.35rem' }}>
            Teachers and Administrators can create course modules in the Course Manager.
          </p>
        </div>
      ) : (
            ...
            {(currentCourse.chapters || []).map(chapter => (
              <div key={chapter.id} className="glass-card">
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.65rem' }}>
                  {(chapter.lessons || chapter.items || []).map(lesson => (
                    <div 
                      key={lesson.id}
                      style={{
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                        ...
                        <div>
                          ...
                          <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>
                            {lesson.type === 'video' ? `Video Lecture • ${lesson.duration || '15 mins'}` : `PDF Notes • ${lesson.fileSize || '1.5 MB'}`}
                          </span>
                        </div>
                      </div>
                    ...
                    <div style={{ background: 'rgba(245, 158, 11, 0.1)', border: '1px solid rgba(245, 158, 11, 0.3)', borderRadius: 'var(--radius-md)', padding: '0.85rem 1rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: '0.5rem' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                        ...
                        <div>
                          ...
                          <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>
                            {chapter.quiz.questionsCount || chapter.quiz.questions?.length || 2} Questions • Time Limit: {chapter.quiz.timeLimit || '10 mins'}
                          </span>
                        </div>
                      </div>
                      <button 
            ...
            <div className="glass-card">
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', fontSize: '0.85rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  ...
                  <strong>{currentCourse.teacher || 'Faculty Staff'}</strong>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ color: 'var(--text-muted)' }}>Class Code:</span>
                  <strong>{currentCourse.code || 'CRS-901'}</strong>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
      ...
      {/* Video Modal Player */}
      {activeVideoModal && (
        <div className="modal-overlay" onClick={(e) => e.target === e.currentTarget && setActiveVideoModal(null)}>
          <div className="modal-container" style={{ maxWidth: '800px', padding: '1.5rem' }}>
            ...
            <div style={{ width: '100%', height: '400px', background: '#000', borderRadius: 'var(--radius-md)', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <iframe 
                width="100%" 
                height="100%" 
                src={activeVideoModal.videoUrl || "https://www.youtube-nocookie.com/embed/dQw4w9WgXcQ"} 
      ...
      {/* PDF Viewer Modal */}
      {activePdfModal && (
        <div className="modal-overlay" onClick={(e) => e.target === e.currentTarget && setActivePdfModal(null)}>
          <div className="modal-container" style={{ maxWidth: '720px', padding: '1.5rem' }}>
            ...
            <div style={{ background: 'var(--bg-card-hover)', padding: '2rem', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-color)', minHeight: '300px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center' }}>
              ...
              <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem', margin: '0.5rem 0 1.5rem 0' }}>
                File Size: {activePdfModal.fileSize || '1.8 MB'} • High Resolution Digital Study Notes
              </p>
      ...
      {showSubmitModal && (
        <div className="modal-overlay" onClick={(e) => e.target === e.currentTarget && setShowSubmitModal(null)}>
          <div className="modal-container" style={{ padding: '1.5rem' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              <input 
                type="text" 
                placeholder="Enter submission notes or attachment name (e.g. Solution_Ch2.pdf)..."
                value={fileInputName}
